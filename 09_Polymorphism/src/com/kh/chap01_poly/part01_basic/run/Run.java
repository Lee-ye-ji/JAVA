...
public class Run {
	...
	public static void main(String[] args) {
		// 원칙 : '=' 라는 대입 연산자의 양 옆 자료형(타입)은 같아야 함
		...
		// 대입 연산자를 기준으로 왼쪽과 오른쪽 타입이 다르지만 문제 없음 -> 자동 형변환, 목시적 형변환
		p2.printParent();
		// p2라는 레퍼런스로 Parent에 접근 가능 -> 현재 Parent타입이므로
		 ...
		 * 자식타입 -> 부모타입
		 * Parent p = new Child1();
		 * 형 변환 표시 생략 가능 => 자동 형변환, 목시적 형변환
		 *  
		 * 2. DownCasting
		 * 부모타입 -> 자식타입
		 * ((Child1)p2).printChild1();
		 * 형변환 생략 불가능 => 강제 형변환, 명시적 형변환
		 */
		...
		// 런타임 에러 : 프로그램 실행 시 발생하는 오류  => ClassCastException
		
		// 다형성의 정의
		// : 부모 타입으로부터 파생된 여러가지 타입의 자식 객체를 부모 클래스 타입 하나로 다룰 수 있는 기술
		// 다형성을 쓰는 이유?
		...
		for(int i = 0; i < arr.length; i++) {
			/* 각 인덱스 별로 어떤 자식 객체를 참조하고 있는지 어떻게 판별하지?
			 * < instanceof 연산자 >
			 * 현재 레퍼런스가 어떤 클래스 형의 주소를 참조하고 있는지 확인할 때 사용
		...
		System.out.println("=== 향상된 for문 이용해서 출력 ===");
		for(Parent p : arr) {
		...
		for(int i = 0; i < arr.length; i++) {
			...
			// 그러나 메소드 실제 호출 시 해당 객체 타입을 기준으로 다시 바인딩 되는 것을 "동적 바인딩"이라고 함
			// 상속 관게로 이루어져 다형성이 적용된 경우 메소드 오버라이딩이 되어있으면
