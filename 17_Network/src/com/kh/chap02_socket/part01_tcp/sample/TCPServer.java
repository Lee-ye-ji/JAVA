...
public class TCPServer {
	public void serverStart() {
		...
		// 0 ~ 66535사이에서 지정 가능(단, 1023번 이하는 이미 사용 중인 포트가 많으니 이외의 포트 지정)
		
		try {
			// 2) ServerSocket 객체 생성 후 포트 결합
			ServerSocket server = new ServerSocket(port);
			
			// 3) 클라이언트부터 접속 요청이 올 때까지 대기
			...
			// 소켓은 두 개의 스트림(입/출력)을 가지고 있으며 이 스트림은 연결된 상대편 소켓의 스트림과 교차 연결 됨
			InputStream input = client.getInputStream(); // -> 클라이언트 소켓의 OutputStream과 연결됨
